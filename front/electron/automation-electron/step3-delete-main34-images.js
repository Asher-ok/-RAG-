 ...
 */
async function deleteMain34Images(window) {
  console.log(`\n========== [步骤3] 删除主图3:4区域的所有图片 ==========`);
  
  try {
    // 1. 滚动到主图3:4区域
    console.log(`[步骤3.1] 定位到主图3:4区域...`);
      ...
      try {
        ...
        const result = await executeJS(window, `
          (function() {
            ...
            const main34Section = document.querySelector('[attr-field-id="主图3:4"]');
            if (!main34Section) {
              return { finished: true, count: 0, reason: '未找到主图3:4区域' };
            }
            
            // 查找该区域内的所有图片容器
            const imageWrappers = main34Section.querySelectorAll('div.index-module_imgWrapper__xOFF7');
            ...
            console.log('[删除主图3:4] 当前还有', count, '张图片');
                ...
                try {
                  ...
                  if (deleteIcon) {
                    ...
                    if (deleteButton) {
                      deleteButton.click();
                      deleted = true;
                      console.log('[删除主图3:4] ✓ 已删除第', count, '张图片（方法1）');
                      return true;
                    }
                  }
                } catch (error) {
                  ...
                  try {
                    ...
                    for (const selector of deleteButtonSelectors) {
                      ...
                      if (deleteButtons && deleteButtons.length > 0) {
                        ...
                        deleted = true;
                        console.log('[删除主图3:4] ✓ 已删除第', count, '张图片（方法2）');
                        return true;
                      }
                    }
                  } catch (error) {
                ...
                if (!deleted) {
                  try {
                    ...
                    if (deleteIcons && deleteIcons.length > 0) {
                      ...
                      deleted = true;
                      console.log('[删除主图3:4] ✓ 已删除第', count, '张图片（方法3）');
                      return true;
                    }
                  } catch (error) {
                    // ignore
    ...
    const verifyResult = await executeJS(window, `
      (function() {
        const main34Section = document.querySelector('[attr-field-id="主图3:4"]');
        if (!main34Section) {
          return { success: false, reason: '未找到主图3:4区域' };
        }
        ...
        // 检查是否显示上传按钮
        const uploadButtons = main34Section.querySelectorAll('label.index-module_button__st1_R');
        const uploadButtonCount = uploadButtons ? uploadButtons.length : 0;
        
        // 检查是否有"商品正面图"和"上传辅助图"按钮
    ...
    console.error(`  ✗ 删除主图3:4失败: ${error.message}`);
    console.error(error.stack);
    return { 
      success: false, 
      message: `删除主图3:4失败: ${error.message}` 
    };
  }
}
